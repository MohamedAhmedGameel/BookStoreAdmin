...
function AddProduct({ setModify, book }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const authorRes = await axios.get('https://moga-library.000webhostapp.com//api/author.php');
                setAuthors(authorRes.data);

                const categoryRes = await axios.get('https://moga-library.000webhostapp.com//category.php');
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting, resetForm }) => {
            if (!book) {
                try {
                    const res = await axios.post('https://moga-library.000webhostapp.com//api/action.php', values);
